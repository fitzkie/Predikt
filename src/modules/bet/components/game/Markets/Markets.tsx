...
import { useBetBetsSummaryBySelection, useBetConditionState, useBetGameMarkets, useBetResolvedMarkets } from 'modules/bet/hooks'
...
const Condition: React.FC<ConditionProps> = (props) => {
  ...
  const { data: state, isLocked } = useBetConditionState({
    conditionId,
    initialState,
  })
...
const ResolvedMarkets: React.FC<MarketsProps> = ({ game, gameState }) => {
  ...
  const { data: markets, isLoading } = useBetResolvedMarkets({ gameId: game.gameId })
  const { data: betsSummary } = useBetBetsSummaryBySelection({
    account: address!,
    gameId: game.gameId,
    gameState,
  })
...
const ActiveMarkets: React.FC<MarketsProps> = ({ game, gameState }) => {
  const { data: markets, isFetching: isLoading, isPlaceholderData } = useBetGameMarkets(game.gameId, true)
